...
ordered_verifiers = ['VERL','Qwen', 'HuggingFace',]
...
for _, row in df.iterrows():
    ...
    for i, task in enumerate(all_tasks):
        # Convert FNR to Recall: Recall = 1 - FNR
...
indices = np.arange(0, len(ordered_tasks) * 0.83, 0.83)  # Using 0.7 as step size instead of 1.0
...
ax.set_ylim(0.75, 1.04)  # Keep the display range
# Set custom y-ticks that only go up to 1.0
ax.set_yticks([0.75, 0.80, 0.85, 0.90, 0.95, 1.00])
# Add grid
ax.grid(axis='both', linestyle='--', alpha=0.3)

# Add legend with custom styling, with reduced spacing between columns
...
plt.tight_layout()
plt.savefig(f'Figures/{script_name}_recall_bar.png', dpi=500, bbox_inches='tight')
plt.savefig(f'Figures/{script_name}_recall_bar.pdf', dpi=500, bbox_inches='tight')
plt.show() 