...
'''
    ...
    elif prompt_type == "tiger-verifier":
        prompt = (
...
class GenRMRewardManager:
    ...
    def rule_based_reward(self, data: DataProto, preprocess_for_genrm: bool = False) -> Union[torch.Tensor, dict]:
        ...
        for i in range(len(data)):
            ...
            if not self.config.get("model_based_reward_only", False):
                ...
                extracted_answer, is_boxed_matched = hf_extract_solution(sequences_str)
                score_result = {"score": 0.0, "extracted_answer": extracted_answer, "extra_info": {"is_boxed_matched": is_boxed_matched}}
            ...
            #     extracted_answer_list.append('')  # Empty string for extracted answer if not provided
        ...
        if preprocess_for_genrm:
            ...
            if need_genrm_evaluation:
                # breakpoint()
                # Create filtered lists with only the items needing genRM
                # filtered_extracted_answers = [extracted_answer_list[i] for i in need_genrm_evaluation]
                # filtered_ground_truths = [ground_truth_list[i] for i in need_genrm_evaluation]
                # breakpoint()
