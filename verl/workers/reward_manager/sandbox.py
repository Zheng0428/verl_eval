...
from sandbox_fusion import set_sandbox_endpoint, run_code_async, RunCodeRequest, RunStatus
...
async def single_sandbox_fusion_async(code, language='python', compile_timeout=1.0, run_timeout=3.0, semaphore=None):
    ...
    if code is None:
        ...
        async with semaphore:
            try:
                response = await run_code_async(request, client_timeout=5.)
...
async def parallel_sandbox_fusion_async(evaluation_func, completions, references, tasks, num_processes=64, sandbox_url=None):
...
class SandboxRewardManager:
    ...
    def __init__(self, tokenizer, num_examine, compute_score=None, sandbox_url=None) -> None:
        ...
        self.compute_score = compute_score or _default_compute_score # TODO: change to different compute scores (sandbox) later.
        if sandbox_url is None:
    ...
    def __call__(self, data: DataProto):
        ...
        try:
            scores = asyncio.run(
                parallel_sandbox_fusion_async(self.compute_score,
                                             sequences_str,
                                             ground_truth,
                                             data_sources,
                                             num_processes=64,
        ...
        for i in range(len(data)):
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                if type(sequences_str) is list:
                    print("There are {} sequences_str".format(len(sequences_str)))
