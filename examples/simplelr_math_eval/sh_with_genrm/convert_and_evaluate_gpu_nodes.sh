...
# 参数检查
# if [ "$#" -gt 21 ]; then
#     echo "Usage: $0 <eval_script_path> <base_checkpoint_path> <init_model_path> <template> [benchmarks] [temperature] [max_tokens] [top_p] [tp_size] [ckpt_list_file] [output_dir] [overwrite] [n_sampling] [convert_model] [split] [n_test_sample] [judge_model_path] [judge_temperature] [judge_top_p] [judge_max_tokens] [judge_template]"
#     exit 1
# fi
...
n_test_sample=${16:--1}
judge_model_path=${17:-""}
judge_temperature=${18:-0.0}
judge_top_p=${19:-0.9}
judge_max_tokens=${20:-32000}
judge_template=${21:-"tiger-verifier"}
...
convert_checkpoint() {
    ...
    # Run conversion
    CUDA_VISIBLE_DEVICES=$gpu_ids python3 sh_with_genrm/model_merger.py \
        --backend fsdp \
...
process_checkpoint() {
    ...
    CUDA_VISIBLE_DEVICES=$gpu_ids bash "$eval_script_path" ${template} "$ckpt_path" "$output_path_new" "$temperature" "$max_tokens" "$top_p" "$benchmarks" "$overwrite" "$n_sampling" "$split" "$n_test_sample" "$judge_model_path" "$judge_temperature" "$judge_top_p" "$judge_max_tokens" "$judge_template"
}
