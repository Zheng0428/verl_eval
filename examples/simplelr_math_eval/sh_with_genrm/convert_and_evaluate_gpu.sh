...
get_checkpoints_to_evaluate() {
    ...
    for ckpt_dir in "$base_path"/global_step_*; do
        if [ -d "$ckpt_dir" ]; then
            ...
            # 检查是否已经评估过
            # if [ ! -f "$base_path/eval_results/$step_tag/eval_done.txt" ]; then
            #     checkpoints+=("$step_tag")
            # else
            #     echo "Skipping $step_tag: already evaluated" >&2
            # fi
        fi
    done
...
process_checkpoint() {
    ...
    if [ -d "$base_checkpoint_path/$step_tag/$actor_dir" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$gpu_ids bash "$eval_script_path" ${template} "$ckpt_path" "$output_path_new" "$temperature" "$max_tokens" "$top_p" "$benchmarks" "$overwrite"
}
