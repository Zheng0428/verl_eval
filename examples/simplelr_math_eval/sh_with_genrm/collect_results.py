...
def process_file(args):
    ...
    try:
        ...
        with open(metrics_file, 'r') as f:
            ...
            # Add the new metrics
            rule_based_reward = metrics.get('rule_based_reward', 0)
            hybrid_reward = metrics.get('hybrid_reward', 0)
            model_only_reward = metrics.get('model_only_reward', 0)
            rule_and_hybrid_gap = metrics.get('rule_and_hybrid_gap', 0)
        ...
        return model_name_norm, benchmark, {
            ...
            'model_only_reward': model_only_reward,
            'rule_and_hybrid_gap': rule_and_hybrid_gap
        }
...
def collect_results(base_dir, model_name, num_threads=8, temperature=None):
    ...
    metrics_files = glob.glob(f"{base_dir}/**/train_*_verifier_*_metrics.json", recursive=True)
...
def create_summary(results):
    ...
    for model, benchmarks in results.items():
        ...
        total_stop_ratio = 0
        total_box_ratio = 0
        total_repeat_ratio = 0
        # Add totals for new metrics
        total_rule_based_reward = 0
        total_hybrid_reward = 0
        total_model_only_reward = 0
        total_rule_and_hybrid_gap = 0
        count = 0
        ...
        for benchmark, metrics in benchmarks.items():
            ...
            row[f'{benchmark}_box_ratio'] = metrics['box_ratio']
            row[f'{benchmark}_repeat_ratio'] = metrics['repeat_ratio']
            
            # Add the new reward metrics
            row[f'{benchmark}_rule_based_reward'] = metrics['rule_based_reward']
            row[f'{benchmark}_hybrid_reward'] = metrics['hybrid_reward']
            row[f'{benchmark}_model_only_reward'] = metrics['model_only_reward']
            row[f'{benchmark}_rule_and_hybrid_gap'] = metrics['rule_and_hybrid_gap']
            ...
            total_repeat_ratio += metrics['repeat_ratio']
            
            # Accumulate new metrics
            total_rule_based_reward += metrics['rule_based_reward']
            total_hybrid_reward += metrics['hybrid_reward']
            total_model_only_reward += metrics['model_only_reward']
            total_rule_and_hybrid_gap += metrics['rule_and_hybrid_gap']
            count += 1
        ...
        if count > 0:
            ...
            row['avg_box_ratio'] = total_box_ratio / count
            row['avg_repeat_ratio'] = total_repeat_ratio / count
            
            # Add averages for new metrics
            row['avg_rule_based_reward'] = total_rule_based_reward / count
            row['avg_hybrid_reward'] = total_hybrid_reward / count
            row['avg_model_only_reward'] = total_model_only_reward / count
            row['avg_rule_and_hybrid_gap'] = total_rule_and_hybrid_gap / count
...
def plot_training_progress(results, output_dir, benchmarks=None):
    ...
    # Add plots for reward metrics
    for plot_type in ['reward_metrics', 'reward_gap']:
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(15, 5*n_rows))
        fig.suptitle(f'Training Progress - {base_name} - {plot_type.replace("_", " ").title()}')
        axes = axes.flatten()
        ...
        for model in models:
            ...
            for benchmark in all_benchmarks:
                if benchmark in metrics:
                    model_rule_based.append(metrics[benchmark].get('rule_based_reward', 0))
                    model_hybrid.append(metrics[benchmark].get('hybrid_reward', 0))
                    model_model_only.append(metrics[benchmark].get('model_only_reward', 0))
                    model_gap.append(metrics[benchmark].get('rule_and_hybrid_gap', 0))
            
            avg_metrics['rule_based_reward'].append(sum(model_rule_based) / len(model_rule_based) if model_rule_based else 0)
            avg_metrics['hybrid_reward'].append(sum(model_hybrid) / len(model_hybrid) if model_hybrid else 0)
            avg_metrics['model_only_reward'].append(sum(model_model_only) / len(model_model_only) if model_model_only else 0)
            avg_metrics['rule_and_hybrid_gap'].append(sum(model_gap) / len(model_gap) if model_gap else 0)
        
        # Plot first subplot (average)
        ax = axes[0]
        
        if plot_type == 'reward_metrics':
            ax.set_title('Average Reward Metrics')
            ax.set_xlabel('Checkpoint')
            ax.set_ylabel('Reward Value')
            
            line1 = ax.plot(range(len(checkpoints)), avg_metrics['rule_based_reward'], marker='o', color='#1f77b4', label='Rule Based')
            line2 = ax.plot(range(len(checkpoints)), avg_metrics['hybrid_reward'], marker='s', color='#ff7f0e', label='Hybrid')
            line3 = ax.plot(range(len(checkpoints)), avg_metrics['model_only_reward'], marker='^', color='#2ca02c', label='Model Only')
            ...
            # Add value annotations
            for i, (v1, v2, v3) in enumerate(zip(avg_metrics['rule_based_reward'], 
                                              avg_metrics['hybrid_reward'], 
                                              avg_metrics['model_only_reward'])):
                ax.annotate(f'{v1:.2f}', (i, v1), textcoords="offset points", xytext=(0,10), ha='center', color='#1f77b4', fontsize=8)
                ax.annotate(f'{v2:.2f}', (i, v2), textcoords="offset points", xytext=(0,-15), ha='center', color='#ff7f0e', fontsize=8)
                ax.annotate(f'{v3:.2f}', (i, v3), textcoords="offset points", xytext=(15,0), ha='center', color='#2ca02c', fontsize=8)
            
            lines = line1 + line2 + line3
            labels = [l.get_label() for l in lines]
            ax.legend(lines, labels, loc='upper left')
        else:  # rule_and_hybrid_gap
            ax.set_title('Average Rule-Hybrid Gap')
            ax.set_xlabel('Checkpoint')
            ax.set_ylabel('Gap Value')
            
            line1 = ax.plot(range(len(checkpoints)), avg_metrics['rule_and_hybrid_gap'], marker='o', color='#d62728', label='Rule-Hybrid Gap')
            ...
            # Add value annotations
            for i, v1 in enumerate(avg_metrics['rule_and_hybrid_gap']):
                ax.annotate(f'{v1:.2f}', (i, v1), textcoords="offset points", xytext=(0,10), ha='center', color='#d62728', fontsize=8)
        ...
        for i, benchmark in enumerate(all_benchmarks, start=1):
            ...
            if plot_type == 'reward_metrics':
                ...
                for model in models:
                    metrics = results[model].get(benchmark, {})
                    rule_based_values.append(metrics.get('rule_based_reward', 0))
                    hybrid_values.append(metrics.get('hybrid_reward', 0))
                    model_only_values.append(metrics.get('model_only_reward', 0))
                
                line1 = ax.plot(range(len(checkpoints)), rule_based_values, marker='o', color='#1f77b4', label='Rule Based')
                line2 = ax.plot(range(len(checkpoints)), hybrid_values, marker='s', color='#ff7f0e', label='Hybrid')
                line3 = ax.plot(range(len(checkpoints)), model_only_values, marker='^', color='#2ca02c', label='Model Only')
                
                ax.set_xticks(range(len(checkpoints)))
                ax.set_xticklabels(checkpoints, rotation=45)
                ax.grid(True, alpha=0.3)
                
                for j, (v1, v2, v3) in enumerate(zip(rule_based_values, hybrid_values, model_only_values)):
                    ax.annotate(f'{v1:.2f}', (j, v1), textcoords="offset points", xytext=(0,10), ha='center', color='#1f77b4', fontsize=8)
                    ax.annotate(f'{v2:.2f}', (j, v2), textcoords="offset points", xytext=(0,-15), ha='center', color='#ff7f0e', fontsize=8)
                    ax.annotate(f'{v3:.2f}', (j, v3), textcoords="offset points", xytext=(15,0), ha='center', color='#2ca02c', fontsize=8)
                ...
                for model in models:
                    metrics = results[model].get(benchmark, {})
                    gap_values.append(metrics.get('rule_and_hybrid_gap', 0))
                
                line1 = ax.plot(range(len(checkpoints)), gap_values, marker='o', color='#d62728', label='Rule-Hybrid Gap')
                
                ax.set_xticks(range(len(checkpoints)))
                ax.set_xticklabels(checkpoints, rotation=45)
                ax.grid(True, alpha=0.3)
                
                for j, v1 in enumerate(gap_values):
                    ax.annotate(f'{v1:.2f}', (j, v1), textcoords="offset points", xytext=(0,10), ha='center', color='#d62728', fontsize=8)
