...
def calculate_metrics(predictions, ground_truth, org_example_num, current_example_num):
    if predictions and ground_truth:
        ...
        # Calculate FNR for the entire system
        ...
        # Calculate for examples classified as true by rule-based
        ...
        # Calculate FNR
        fnr = total_false_negatives / total_actual_positives if total_actual_positives > 0 else 0
        print(f"\nFalse Negative Rate (FNR): {fnr:.4f} ({fnr*100:.2f}%)")
        
        # Calculate current model's contribution to reducing FNR
        if class_1_total > 0:
            recovered_positives_percentage = true_positives_current / class_1_total * 100
            print(f"Percentage of positives recovered by model classifier: {recovered_positives_percentage:.2f}%")
        
        return acc, rec, f1, cm, fnr
    else:
...
if __name__ == "__main__":
    ...
    for org, test in zip(org_data, test_data):
        ...
        # print(pred)
        if pred is not None:
            match = re.search(ext_re, pred, re.IGNORECASE)
            if match:
                extracted_answer = match.group(1).strip().lower()
                if extracted_answer.lower().strip()  in ["yes", "true"]:
                    new_pred = 1.0
                elif extracted_answer.lower().strip() in ["no", "false"]:
                    new_pred = 0.0
                else:
                    new_pred = 0.0
            else:
